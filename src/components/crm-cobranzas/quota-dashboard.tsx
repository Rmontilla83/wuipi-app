...
export default function QuotaDashboard() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/crm-cobranzas/quotas/progress?month=${month}`)
      .then(r => r.json())
    ...
    <Card>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-white flex items-center gap-2">
          <Trophy size={16} /> Cuotas de Cobradores
        </h3>
          ...
          {progress.map(q => (
            <div key={q.id} className="p-3 bg-wuipi-bg rounded-lg border border-wuipi-border">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-white">{q.crm_collectors?.full_name || "—"}</span>
                <span className="text-[10px] text-gray-500">{q.crm_collectors?.type === "external" ? "Externo" : "Interno"}</span>
              </div>
              {/* Count bar */}
              <div className="mb-2">
                <div className="flex items-center justify-between text-[10px] mb-1">
                  <span className="text-gray-500 flex items-center gap-1"><Target size={10} /> Recuperaciones</span>
              ...
              <div>
                <div className="flex items-center justify-between text-[10px] mb-1">
                  <span className="text-gray-500">$ Monto Recuperado</span>
          ...
          <div className="pt-2 border-t border-wuipi-border">
            <div className="flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-4">
                <span className="text-white font-bold">{totalActualCount}/{totalTargetCount} recuperaciones</span>
                <span className="text-amber-400 font-bold">{fmtUSD(totalActualAmount)}/{fmtUSD(totalTargetAmount)}</span>
              </div>
            </div>
          </div>
        </div>
